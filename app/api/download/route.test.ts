...
const VALID_RELEASE = {
  id: 'rel-1',
  version: '1.0.2',
  storage_path: 'macos/BuildHarvey-1.0.2.dmg',
  sha256: 'b'.repeat(64),
}

const SIGNED_URL = 'https://storage.example.com/signed?token=xyz'
...
describe('POST /api/download (macOS)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 409 when storage_path does not start with macos/', async () => {
    mockAuth({})
    mockAdmin({ releases: [{ ...VALID_RELEASE, storage_path: 'windows/BuildHarvey-1.0.2.dmg' }] })
    const res = await POST()
    expect(res.status).toBe(409)
    const body = await res.json()
    expect(body.error).toBe('invalid_release_configuration')
  ...
  it('returns 409 when storage_path does not end with .dmg', async () => {
    mockAuth({})
    mockAdmin({ releases: [{ ...VALID_RELEASE, storage_path: 'macos/BuildHarvey-1.0.2.exe' }] })
    const res = await POST()
    expect(res.status).toBe(409)
    const body = await res.json()
    expect(body.error).toBe('invalid_release_configuration')
